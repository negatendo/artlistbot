$imported_categories = {
  "superlatives" => [
    "Hot off the list presses:",
    "List update!",
    "Time to make the lists!",
    "I'mmmm updating liiiiiists! :D",
    "Congratulations!",
    "Oh no!",
    "Uh oh!",
    "Interesting.",
    "This is new.",
    "Wow.",
    "This just in!",
    "Important list update!",
    "I've updated my lists!",
    "Add the 8... carry the 0... ok so",
    "According to my calculations",
    "Commiserations!",
    "Breaking news",
    "Sorry to inform you",
    "Hmmm",
    "As it stands",
    "It's all changed.",
    "It's official!",
    "UPDATE!",
    "Just updated:",
    "So this just happened:"
  ],
  "upward_verbs" => [
    "has risen",
    "has moved up",
    "just ranked up",
    "ranked up",
    "has increased their standing",

    "has been moved",
    "may now be recognized as changing",
    "should be seen as changing",
    "has taken a new position",
    "is having their listing changed",
  ],
  "downward_verbs" => [
    "has fallen to",
    "has dropped down",
    "just ranked down to",
    "ranked down",
    "has decreased their standing to the",

    "has been moved",
    "may now be recognized as changing",
    "should be seen as changing",
    "has taken a new position",
    "is having their listing changed",
  ],
  "neutral_verbs" => [
    "has been set to",
    "may now be recognized as",
    "can now be seen as",
    "has taken a new position as",
    "is having their listing set at",
    "is now being tracked, settling in at",
  ],
  "names_for_lists" => [
    "on the list:",
    "on my list:",
    "on this here thing that lists the",
    "with my list",
    "on this list:",
    "on the following list:"
  ],
  "adjectives" => [
    "most likely",
    "least likely",
    "most promenient",
    "least promenient",
    "biggest",
    "smallest",
    "tiniest",
    "most often to",
    "least often to",
    "cutest",
    "hottest",
    "most attractive",
    "most ridiculous",
    "strongest",
    "tallest",
    "smartest",
    "sharpest",
    "most oblivious",
    "ugliest",
    "most relevant",
    "coolest",
    "most on point",
    "realest",
    "illest"
  ],
  "nouns" => [
    "net artists",
    "bloggers",
    "user accounts",
    "twitter accounts",
    "facebook users",
    "instagram users",
    "HTML coders",
    "painters",
    "creatives",
    "nerds",
    "artists",
    "art fans",
    "curators",
    "interns",
    "hackers",
    "new media artists",
    "art lovers",
    "craft enthusiasts",
    "cat lovers",
    "creative geniuses",
    "innovators",
    "visionaires",
    "design gurus",
    "procrastinators",
    "attention seekers",
    "trolls",
    "spammers"
  ],
  "categories" => [
    "to be on Twitter",
    "to miss your private view",
    "who is also Spammer of the year",
    "who is also Meme lover of the year",
    "who is also Meme hater of the year",
    "to take a museum selfie",
    "to follow @jerrysaltz",
    "to sell their art",
    "treat a residency like a holiday",
    "to unfollow you",
    "Post-internet artist of the year",
    "to ask But is it Art?",
    "to check their website stats",
    "to send an email without the attachment",
    "to be online",
    "to have an exhibition",
    "to share your content",
    "to have their work featured",
    "to share your content",
    "to have their work featured",
    "to buy followers",
    "to go viral",
    "with the most aesthetically pleasing art",
    "to make a twitter bot",
    "to quit the internet",
    "to delete their account",
    "to hoard collectable art",
    "to lose their art",
    "to delete their art",
    "that likes Banksy",
    "to have a cat",
    "that uses emojis",
    "that really made our year",
    "to delete a post if it received no likes",
    "that remembers before the iPhone",
    "that you should be blocking",
    "that you really must follow",
    "there is"
  ]
}
